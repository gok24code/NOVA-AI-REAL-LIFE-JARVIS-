...
// the wrist than its PIP joint — works regardless of hand rotation, unlike
// a simple tip.y < pip.y check.
const EXTENDED_MARGIN = 1.15;

// How long a fist/three-finger pose must hold before it fires again —
// otherwise a single held gesture would retrigger every frame (~60/s).
...
export class HandTracker {
  ...
  private checkFingerGestures(
    ...
    const indexExt = dist2d(wrist, lm[INDEX_TIP]) > dist2d(wrist, lm[INDEX_PIP]) * EXTENDED_MARGIN;
    const middleExt = dist2d(wrist, lm[MIDDLE_TIP]) > dist2d(wrist, lm[MIDDLE_PIP]) * EXTENDED_MARGIN;
    const ringExt = dist2d(wrist, lm[RING_TIP]) > dist2d(wrist, lm[RING_PIP]) * EXTENDED_MARGIN;
    const pinkyExt = dist2d(wrist, lm[PINKY_TIP]) > dist2d(wrist, lm[PINKY_PIP]) * EXTENDED_MARGIN;
    const count = [indexExt, middleExt, ringExt, pinkyExt].filter(Boolean).length;

    const now = performance.now();
    if (now - state.lastGestureAt < GESTURE_COOLDOWN_MS) return;
